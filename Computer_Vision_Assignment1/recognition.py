...
def binarize(image, **binarization_kwargs):
    ...
    binarized_image = cv2.adaptiveThreshold(image,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,31,19)
...
def get_sudoku_cells(frontalized_image, crop_factor=0.42, binarization_kwargs={}):
    ...
    for i in range(NUM_CELLS):
        for j in range(NUM_CELLS):
            sudoku_cell = binarized_image[i*CELL_SIZE[0]:(i+1)*CELL_SIZE[0], j*CELL_SIZE[1]:(j+1)*CELL_SIZE[1]]
...
def is_empty(sudoku_cell, **kwargs):
    ...
    cell_is_empty = np.mean(sudoku_cell) > 240
...
def get_digit_correlations(sudoku_cell, templates_dict):
    ...
    for digit, templates in templates_dict.items():
        ...
        correlations[digit - 1] = np.array([skimage.feature.match_template(sudoku_cell, templates[0], pad_input=True, mode='constant' ,constant_values=255),skimage.feature.match_template(sudoku_cell, templates[1], pad_input=True, mode='constant' ,constant_values=255)]).max()
    if 0==correlations.argmax() and (correlations[correlations.argsort()[-1]]-correlations[correlations.argsort()[-2]])<0.1:
        correlations[0]=0.5
    return correlations
...
def recognize_digits(sudoku_cells, templates_dict, threshold=0.5):
    ...
    for i in range(sudoku_cells.shape[0]):
        for j in range(sudoku_cells.shape[1]):
            cor=get_digit_correlations(sudoku_cells[i, j], templates_dict)
            if cor.max()<threshold:
                sudoku_matrix[i, j] = 0
            else:
                sudoku_matrix[i, j] = cor.argmax()+1
